                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">設備清單</h6>
                        </div>
                        <div class="card-body">
...
<%-- 																						<th scope="row" style="text-align: center;">${id.count}</th> --%>
																						<td id="thnb${device.did}">${device.number }</td>
																						<td id="thlc${device.did}">${device.location }</td>
																						<td id="thci${device.did}">${device.coin }</td>
																						<td id="thsv${device.did}">${device.coin }</td>
																						<td id="thpp${device.did}">${device.paper }</td>
																						<td id="thcc${device.did}"><button class="btn btn-warning">執行清空</button></td>
																						<td id="thfc${device.did}"><button class="btn btn-primary">服務內容</button></td>
																						<td id="thrc${device.did}"><button class="btn btn-info">查詢</button></td>
																					</tr>
																				</c:forEach>
                                    </tbody>
                                </table>
            ...
            <div class="form-popup" id="locationForm" >
            	<div  class="form-container" style="background-color: #BFFFDF;">
            		...
            		<div>
            				<label style="width:32%; float:left; margin-top: 7px;"><b>欲更新店名</b></label>
						    		<input style="width:68%; float:left; height: 24px;" type="text" name="location" id="inputlocation">
            		</div>
						    <button type="submit" class="btn" style="height: 48px; padding: 8px;" id="updateLocationBtn">更新</button>
						    <button type="submit" class="btn cancel"  style="height: 48px; padding: 8px;" onclick="closeAllForm()">取消</button>
            	</div>
						</div>
            	...
            	<div  class="form-container" style="background-color: #BFFFDF;">
            		...
            		<div>
            				<label style="width:32%; float:left; margin-top: 7px;"><b>欲退幣數量</b></label>
						    		<input style="width:68%; float:left; height: 24px;" type="text" name="dropout" id="inputdropout">
            		</div>
						    <button type="submit" class="btn" style="height: 48px; padding: 8px;" id="updateDropoutBtn">退幣</button>
						    <button type="submit" class="btn cancel"  style="height: 48px; padding: 8px;" onclick="closeAllForm()">取消</button>
            	</div>
						</div>
            	...
            	<div  class="form-container" style="background-color: #BFFFDF;">
            		...
            		<div>
            				<label style="width:32%; float:left; margin-top: 7px;"><b>欲設定儲幣</b></label>
						    		<input style="width:68%; float:left; height: 24px;" type="text" name="coinset" id="inputcoinset">
            		</div>
						    <button type="submit" class="btn" style="height: 48px; padding: 8px;" id="updateCoinsetBtn">設定</button>
						    <button type="submit" class="btn cancel"  style="height: 48px; padding: 8px;" onclick="closeAllForm()">取消</button>
            	</div>
						</div>
            	...
            	<div  class="form-container" style="background-color: #BFFFDF;">
            		...
            		<div>
						    ...
						    <button type="submit" class="btn" style="height: 48px; padding: 8px;" id="updateCleanpaperBtn">清空</button>
						    <button type="submit" class="btn cancel"  style="height: 48px; padding: 8px;" onclick="closeAllForm()">取消</button>
            	</div>
						</div>
            	...
            	<div  class="form-container" style="background-color: #BFFFDF;">
            		...
            		<div>
						    ...
						    <button type="submit" class="btn" style="height: 48px; padding: 8px;" id="updateCleancoinBtn">清空</button>
						    <button type="submit" class="btn cancel"  style="height: 48px; padding: 8px;" onclick="closeAllForm()">取消</button>
            	</div>
						</div>
            	...
            	<div  class="form-container" style="background-color: #BFFFDF;">
            		...
            		<h5 id="freecountNow"><b>免費洗衣或烘衣</b></h5>
								<label style="width:100%; float:left; margin-top: 7px;">輸入機械編號(0 - 63)</label>
								<input style="width:100%; float:left; height: 24px;" type="text" name="coinset" id="inputdevicenumber">
								<label style="width:100%; float:left; margin-top: 7px;">選擇枚數(1 - 30)</label>
						    <input style="width:100%; float:left; height: 24px;" type="text" name="coinset" id="inputfreeset">
						    <button type="submit" class="btn" style="height: 48px; padding: 8px;" id="updateFreecountBtn">執行</button>
						    <button type="submit" class="btn cancel"  style="height: 48px; padding: 8px;" onclick="closeAllForm()">取消</button>
            	</div>
						</div>
...
// 		});
		    ...
		    $('#dataTable tbody tr').on('click', 'td', function (event) {
					var id = $(this).attr('id');
					var num =  id.substring(4, id.length);
					var funStr = id.substring(2, 4);
					$("#hiddendid").val(num);
					console.log("id : " + id);
			    console.log("num : " + num);
			    console.log("funStr : " + funStr);
			    console.log("hiddendid : " + $("#hiddendid").val());

			    closeAllForm();

			    if(funStr == "lc"){			    	
			    	$('#locationID').text( $('#thnb'+num).text() );
			    	$('#locationNow').text( "目前店名 : " + $(this).text() );
			    	document.getElementById("locationForm").style.display = "block";
				  }else if(funStr == "ci"){
					  $('#dropoutID').text( $('#thnb'+num).text() );
					  $('#dropoutINow').text( "目前儲幣 : " + $(this).text() );
			    	document.getElementById("dropoutForm").style.display = "block";
				  }else if(funStr == "sv"){
					  $('#coinsetID').text( $('#thnb'+num).text() );
					  $('#coinsetNow').text( "目前儲幣 : " + $(this).text() );
			    	document.getElementById("coinsetForm").style.display = "block";
				  }else if(funStr == "pp"){
					  $('#cleanpaperID').text( $('#thnb'+num).text() );
					  $('#cleanpaperNow').text( "目前紙鈔 : " + $(this).text() );
			    	document.getElementById("cleanpaperForm").style.display = "block";
				  }else if(funStr == "cc"){
					  $('#cleancoinID').text( $('#thnb'+num).text() );
					  $('#cleancoinNow').text( "目前儲幣 : " + $("#thci"+num).text() );
			    	document.getElementById("cleancoinForm").style.display = "block";
				  }else if(funStr == "fc"){
					  $('#freecountID').text( $('#thnb'+num).text() );
			    	document.getElementById("freecountForm").style.display = "block";
				  }else if(funStr == "rc"){
